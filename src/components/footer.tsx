...
// Simple Icons paths (lucide v1 has no brand icons)
      ...
      <div className="container py-12 grid gap-10 md:grid-cols-3 text-sm">
        ...
        <div className="space-y-3">
          ...
          <ul className="space-y-2 text-muted-foreground">
            ...
            <li><a href={site.resume} className="hover:text-foreground">{site.nav.resume[locale]} (PDF)</a></li>
          </ul>
        </div>
        <div className="space-y-3">
          ...
          <ul className="space-y-2 text-muted-foreground">
            {site.socials.map((s) => (
              <li key={s.href}><a href={s.href} target="_blank" rel="noopener noreferrer me" className="inline-flex items-center gap-2 hover:text-foreground"><svg viewBox="0 0 24 24" className="size-4 fill-current" aria-hidden="true"><path d={icons[s.label]} /></svg>{s.label}</a></li>
            ))}
          </ul>
        </div>
      </div>
