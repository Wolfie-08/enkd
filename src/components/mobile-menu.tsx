...
export function MobileMenu({ label, links }: { label: string; links: { href: string; label: string }[] }) {
  ...
  return (
    <details ref={ref} className="group md:hidden">
      ...
      <nav className="absolute inset-x-0 top-14 border-b border-line bg-background/95 backdrop-blur-md font-mono text-sm uppercase tracking-wider">
        <ul className="container divide-y divide-line">
          {links.map((l) => (
            <li key={l.href + l.label}>
              {/* Static files (the CV) need a plain anchor, not client-side routing. */}
              {l.href.endsWith(".pdf")
                ? <a href={l.href} onClick={close} className="block py-4 text-ink">{l.label}</a>
